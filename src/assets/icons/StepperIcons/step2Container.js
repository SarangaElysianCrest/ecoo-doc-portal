...
const Step2Container = ({ title, active, completed }) => {
	const currentStep = parseInt(sessionStorage.getItem("currentStep"));

	const reject = sessionStorage.getItem("rejectStep2");
	const iconSelection = () => {
		if (reject) {
	...
	return (
		<div
			onClick={() => {
				completed || active
					? window.open(regApprovalStepsRoutes + `step02`, "_self")
					: console.log("Step 02 is disabled");
			}}>
			<svg
				width={250}
				height={50}
